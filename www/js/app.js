...
.controller('AppCtrl', function($scope, $ionicPopup, $ionicPopover, $ionicModal, $ionicListDelegate) {
  ...
  $scope.deletePlayer = function(player) {
    var playerIndex = findGameIndex($scope.currentGameInstance.players,player);
    if( playerIndex === -1 ) {
      return;
    }
    var players = $scope.currentGameInstance.players;
  ...
  $scope.deleteGame = function(game) {
    var gameIndex = findGameIndex($scope.games,game);
    if( gameIndex === -1 ) {
      return;
    }
  ...
  $scope.duplicateGame = function() {
    ...
    if( erroCheckingGame(newInstance, $ionicPopup) < 0 ) {
      return;
    }
  ...
  $scope.createOrEditGame = function() {
    ...
    if( isNewGame ) {
      gameInstance = clone($scope.currentGameInstance);
    }
    //this is error checking
    if( erroCheckingGame(gameInstance, $ionicPopup) < 0 ) {
      return;
    }
...
var erroCheckingGame = function(game, $ionicPopup) {
...
var findGameIndex = function(array,game) {
  for(var i = 0; i < array.length; i++) {
    if( array[i].id === game.id ) {
      return i;
    }
  }
  return -1;
